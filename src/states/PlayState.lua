    ...
    if self.canInput then
        ...
        if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') then
            ...
            elseif math.abs(self.highlightedTile.gridX - x) + math.abs(self.highlightedTile.gridY - y) > 1 then
                ...
                :finish(function()
                    local saveHighlighted = self.highlightedTile
                    local matchesFound = self.board:checkMatches()
                    
                    if not matchesFound then
                        gSounds['error']:play()
                        -- swap tiles back
                        self.highlightedTile = nil
                        local tempX = saveHighlighted.gridX -- BACKUP
                        local tempY = saveHighlighted.gridY -- BACKUP
                        
                        saveHighlighted.gridX = newTile.gridX
                        saveHighlighted.gridY = newTile.gridY

                        newTile.gridX = tempX
                        newTile.gridY = tempY

                        -- swap tiles in the tiles table
                        self.board.tiles[saveHighlighted.gridY][saveHighlighted.gridX] =
                            saveHighlighted
                        ...
                        -- tween coordinates between the two so they swap
                        Timer.tween(0.5, {
                            [saveHighlighted] = {x = newTile.x, y = newTile.y},
                            [newTile] = {x = saveHighlighted.x, y = saveHighlighted.y}
                        })
                    else
...
function PlayState:calculateMatches()
    ...
    if matches then
        ...
        local counter = 0
        -- add score for each match, blank tiles 50 points, the others 75
        for k, match in pairs(matches) do
            local points = 0
            for t, tile in pairs(match) do
                if tile.variety == 1 then
                    points = points + #match * 50
                elseif tile.variety > 1 and tile.variety < 5 then
                    points = points + #match * 75
                elseif tile.variety > 4 then
                    points = points + #match * 100
                end
            end
            self.score = self.score + points / #match
            counter = counter + #match
        end

        -- adds 1 second for a 3 tile match and 2 for higher matches
        if counter == 3 then
            self.timer = self.timer + 3
        elseif counter > 3 then
            self.timer = (self.timer + counter) or 4
        end
